...
parser.add_argument('--positon_aggregator_type', '-pat', type=str, default="exp", choices=["sum", "exp", "mean"],
...
parser.add_argument('--scheduler', type=int, default=15, help='Step size for the scheduler')
...
Path(f"log/{args.prefix}/{args.data}/").mkdir(parents=True, exist_ok=True)
filename = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
fh = logging.FileHandler(f'log/{args.prefix}/{args.data}/{filename}.log')
